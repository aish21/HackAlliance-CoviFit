...
# Read and write video files（Only tested webcam input）
cap = choose_run_mode(args)
video_writer = set_video_writer(cap, write_fps=int(7.0))


# # A txt file that saves joint data for the training process (for training)
...
while cv.waitKey(1) < 0:
    ...
    if has_frame:
        ...
        if (time.time() - start_time) > fps_interval:
            # Calculate the number of frames in the interval process, if the interval is 1 second, it is FPS
